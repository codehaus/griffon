...
eventCopyLibsEnd = { jardir ->
    ...
    if (!isPluginProject) {
        def notifyLibDir = "${getPluginDirForName('notify').file}/lib".toString()
//        if(!(packagingType in ['applet', 'webstart'])) {
            ant.fileset(dir: notifyLibDir, includes: "*.jar").each {
                griffonCopyDist(it.toString(), jardir)
            }
            copyPlatformJars(notifyLibDir, jardir)
            copyNativeLibs(notifyLibDir, jardir)
//        } else {

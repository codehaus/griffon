...
// check to see if we already have a LookandfeelMetouiaGriffonAddon
...
slurpedBuilder1.each() { prefix, v ->
    v.each { builder, views ->
        addonIsSet1 = addonIsSet1 || 'LookandfeelMetouiaGriffonAddon' == builder
    }
}

if (addonIsSet1) {
    println 'Removing LookandfeelMetouiaGriffonAddon from Builder.groovy'
    def builderConfigFile1 = new File("${basedir}/griffon-app/conf/Builder.groovy")
    builderConfigFile1.text = builderConfigFile1.text - "root.'LookandfeelMetouiaGriffonAddon'.addon=true\n"
}

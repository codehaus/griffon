...
// check to see if we already have a LookandfeelTinylafGriffonAddon
...
slurpedBuilder1.each() { prefix, v ->
    v.each { builder, views ->
        addonIsSet1 = addonIsSet1 || 'LookandfeelTinylafGriffonAddon' == builder
    }
}

if (addonIsSet1) {
    println 'Removing LookandfeelTinylafGriffonAddon from Builder.groovy'
    def builderConfigFile1 = new File("${basedir}/griffon-app/conf/Builder.groovy")
    builderConfigFile1.text = builderConfigFile1.text - "root.'LookandfeelTinylafGriffonAddon'.addon=true\n"
}

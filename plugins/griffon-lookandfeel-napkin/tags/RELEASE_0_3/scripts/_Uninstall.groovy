...
// check to see if we already have a LookandfeelNapkinGriffonAddon
...
slurpedBuilder1.each() { prefix, v ->
    v.each { builder, views ->
        addonIsSet1 = addonIsSet1 || 'LookandfeelNapkinGriffonAddon' == builder
    }
}

if (addonIsSet1) {
    println 'Removing LookandfeelNapkinGriffonAddon from Builder.groovy'
    def builderConfigFile1 = new File("${basedir}/griffon-app/conf/Builder.groovy")
    builderConfigFile1.text = builderConfigFile1.text - "root.'LookandfeelNapkinGriffonAddon'.addon=true\n"
}

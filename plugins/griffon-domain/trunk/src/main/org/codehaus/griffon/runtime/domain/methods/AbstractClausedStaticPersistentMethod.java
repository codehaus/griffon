...
public abstract class AbstractClausedStaticPersistentMethod extends AbstractPersistentStaticDynamicMethodInvocation {
    ...
    public AbstractClausedStaticPersistentMethod(GriffonDomainHandler griffonDomainHandler, Pattern pattern, String[] operators) {
    ...
    protected Object invokeInternal(final GriffonDomainClass domainClass, String methodName, Object[] arguments) {
        ...
        List<GriffonMethodExpression> expressions = new ArrayList<GriffonMethodExpression>();
    ...
    private String normalizeMethodName(String methodName, Object[] arguments) {
        return methodName += arguments[0];
    }

    protected Object[] normalizeArguments(Object[] arguments) {
        Object[] args = null;
        if (arguments.length == 2) {
            if (arguments[1] instanceof List) {
                arguments[1] = ((List) arguments[1]).toArray();
            }
            if (arguments[1].getClass().isArray()) {
                args = (Object[]) arguments[1];
            } else {
                args = new Object[]{arguments[1]};
            }
        } else {
    ...
    protected abstract Object doInvokeInternalWithExpressions(GriffonDomainClass domainClass, String methodName, Object[] arguments, List<GriffonMethodExpression> expressions, String operatorInUse);
            ...
            for (int i = 0; i < args.length; i++) {
                ...
                if (prop.getType() == String.class && (args[i] instanceof CharSequence)) {
                    ...
                    try {
                        ...
                        // }
                        // else {
                        args[i] = DefaultTypeTransformation.castToType(args[i], prop.getType());
                        // }
                    } catch (GroovyCastException tme) {
                        ...
                        if (Number.class.isAssignableFrom(args[i].getClass())) {
                            try {
                                args[i] = DefaultTypeTransformation.castToType(args[i].toString(), prop.getType());
                            } catch (GroovyCastException tme1) {

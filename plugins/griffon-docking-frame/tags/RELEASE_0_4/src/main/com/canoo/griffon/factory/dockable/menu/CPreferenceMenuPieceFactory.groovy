 ...
 */
class CPreferenceMenuPieceFactory extends BeanFactory {
    CPreferenceMenuPieceFactory() {
        super(CPreferenceMenuPiece, true)
    }

    @Override
    Object newInstance(FactoryBuilderSupport builder, Object name, Object value, Map attributes) {
        if (value instanceof CControl) {
            value = CPreferenceMenuPiece.setup(value)
        } else {
            def control = attributes.remove('control') ?: CStationFactory.findParentControl(builder.context)
            if (control instanceof CControl)
                value = CPreferenceMenuPiece.setup(control)
        }
        if (value)

 ...
 * 02/19/2006
 *
 * JTextComponentSearcher.java - Handles find/replace operations in an JTextComponent.
...
/**
 * Implementation of the {@code Searcher} interface that targets {@code JTextComponent}.<p>
 * Heavily based in {@code SearchEngine}.
 *
 * @author Robert Futrell (RTextArea 1.0)
 * @author Andres Almiray
 */
public class JTextComponentSearcher implements Searcher {
    /**
     ...
     * text area.
     *
     * @param subject   The text area in which to search.
     ...
     * @see #regexReplace
     */
    public boolean find(JComponent subject, String text,
        ...
        String findIn = getFindInText(component, start, forward);
        ...
        if (regex == false) {
            ...
            if (pos != -1) {
                ...
                selectAndPossiblyCenter(component, pos, pos + text.length());
                return true;
            }
        } else {
            ...
            if (regExPos != null) {
                ...
                selectAndPossiblyCenter(component, regExPos.x, regExPos.y);
                return true;
            }
        }
     ...
     */
    protected String getFindInText(JTextComponent component, int start,
                                   boolean forward) {
        ...
        if (forward) {
            try {
                findIn = component.getText(start,
                        component.getDocument().getLength() - start);
     ...
     */
    public final int getNextMatchPos(String searchFor, String searchIn,
                                     boolean forward, boolean matchCase,
                                     boolean wholeWord) {
     ...
     *         match was found.
     */
    protected final int getNextMatchPosImpl(String searchFor,
     ...
     * @see #getNextMatchPos
     */
    public Point getNextMatchPosRegEx(String regEx,
     ...
     * @see #getNextMatchPos
     */
    protected Object getNextMatchPosRegExImpl(String regEx,
                                              CharSequence searchIn, boolean goForward,
                                              boolean matchCase, boolean wholeWord,
                                              String replaceStr) {
     ...
     * @see #getNextMatchPos
     */
    protected RegExReplaceInfo getRegExReplaceInfo(String regEx,
                                                   String searchIn, boolean goForward,
                                                   boolean matchCase, boolean wholeWord,
                                                   String replacement) {
     ...
     */
    private final boolean isWholeWord(CharSequence searchIn,
                                      int offset, int len) {
     ...
     */
    protected int makeMarkAndDotEqual(JTextComponent component,
                                      boolean forward) {
        Caret c = component.getCaret();
     ...
     * @see #find
     */
    protected boolean regexReplace(JComponent subject, String toFind,
        ...
        // selection.
        Caret c = component.getCaret();
        int start = makeMarkAndDotEqual(component, forward);
        ...
        if (info != null) {
            ...
            selectAndPossiblyCenter(component, matchStart, matchEnd);
     ...
     * @see #find
     */
    public boolean replace(JComponent subject, String toFind,
                           String replaceWith, boolean forward, boolean matchCase,
                           boolean wholeWord, boolean regex)
            throws PatternSyntaxException {
        JTextComponent component = (JTextComponent) subject;
        ...
        if (regex) {
            return regexReplace(component, toFind, replaceWith, forward,
                    matchCase, wholeWord);
        }
        ...
        makeMarkAndDotEqual(component, forward);
        if (find(component, toFind, forward, matchCase, wholeWord, false)) {
     ...
     * @see #find
     */
    public int replaceAll(JComponent subject, String toFind,
        ...
        if (regex) {
            ...
            while (find(component, toFind, true, matchCase,
                    wholeWord, true)) {
     ...
     *
     * @param component The text component whose selection is to be centered.
     * @param start     The start of the range to select.
     * @param end       The end of the range to select.
     */
    private void selectAndPossiblyCenter(JTextComponent component, int start,
                                         int end) {
